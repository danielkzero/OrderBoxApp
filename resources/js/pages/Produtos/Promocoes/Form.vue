<template>
  <ProdutosPageShell :main-tabs="mainTabs" active-main-tab="promocoes">
    <div class="rounded-sm border border-gray-200 bg-white">
      <div class="border-b border-gray-200 px-6 py-4">
        <h1 class="text-2xl font-semibold text-gray-700 uppercase">
          {{ isEdit ? "Alterar promocao" : "Nova promocao" }}
        </h1>
        <p class="mt-1 text-sm text-gray-500">
          Crie aqui uma acao promocional para incentivar a venda de produtos do seu catalogo.
        </p>
      </div>
      ...
      <div class="p-6 space-y-8">
        <section>
          ...
          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 md:col-span-6">
              <label class="field-label">* Nome da promocao</label>
        ...
        <section>
          <div class="flex items-center justify-between mb-3">
            <h2 class="text-3xl leading-8 text-gray-500">PRODUTOS DA PROMOCAO</h2>
            ...
            <table class="min-w-full text-sm">
              <thead class="bg-gray-50 text-gray-700">
                <tr>
                  ...
                  <th class="text-left px-3 py-2 w-44">Desconto (%)</th>
              ...
              <tbody>
                <tr v-for="item in form.itens" :key="item.produto_id" class="border-t border-gray-200">
                  ...
                  <td class="px-3 py-2">
                    <input
                      v-model.number="item.desconto_percentual"
                      type="number"
                      min="0"
                      max="100"
                      step="0.01"
                ...
                <tr v-if="!form.itens.length">
                  <td colspan="4" class="px-3 py-8 text-center text-gray-500">
                    Nenhum produto adicionado na promocao.
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div class="border-t border-gray-200 px-6 py-4 flex items-center gap-3">
        <button type="button" class="btn-primary" :disabled="form.processing" @click="salvar">
          ...
          {{ form.processing ? "Salvando..." : "Salvar promocao" }}
        </button>
        <Link :href="`/${empresaId}/produtos/promocoes`" class="btn-outline">Cancelar</Link>
      </div>
    </div>
  </ProdutosPageShell>
</template>
...
const promocao = page.props.promocao || null;
...
const form = useForm({
  ...
  itens: Array.isArray(promocao?.itens)
    ? promocao.itens.map((item) => ({
...
function salvar() {
  const payload = {
    ...
    itens: form.itens.map((item) => ({
      produto_id: Number(item.produto_id),
      desconto_percentual: item.desconto_percentual === "" || item.desconto_percentual === null
        ? null
        : Number(item.desconto_percentual),
    })),
  };

  if (isEdit && promocao?.id) {
    form.transform(() => payload).put(`/${empresaId}/produtos/promocoes/${promocao.id}`);
    return;
  }

  form.transform(() => payload).post(`/${empresaId}/produtos/promocoes`);
}
</script>
